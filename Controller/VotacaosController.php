...
class VotacaosController extends AppController
{
    ...
    public function index($id = null)
    {
        $grupo = isset($this->request->query["grupo"])
            ...
            : null;
        $item = isset($this->request->query["item"])
            ? $this->request->query["item"]
            : null;
        $tr = isset($this->request->query["tr"])
            ? $this->request->query["tr"]
            : null;
        $item_id = isset($this->request->query["item_id"])
            ? $this->request->query["item_id"]
            : null;
        ...
        if ($grupo and $item_id and $tr and $evento_id) {
            $this->set(
                "votacaos",
                $this->Paginator->paginate("Votacao", [
                    "Votacao.grupo" => $grupo,
        ...
        if ($this->request->is(["post", "put"])) {
            ...
            if ($this->request->data["Votacao"]["id"]) {
                ...
                if (!empty($item_id["Votacao"]["id"])) {
                    $this->request->data["Votacao"]["item_id"] =
                        $item_id["Votacao"]["item_id"];
                } else {
            ...
            if ($this->request->data["Votacao"]["resultado"] == "inclusão") {
                if ($this->request->data["Votacao"]["item_id"]) {
                    ...
                    $item["Item"]["texto"] =
                        $this->request->data["Votacao"]["item_modificada"];
                    if ($this->Item->save($item)) {
                        $this->Flash->success(__("Item atualizada"));
                    }
                } else {
                    $this->Flash->error(
                        __("Inclusão não inserida na tabela Items"),
                    );
                }
            }
            ...
            if (
                substr($this->request->data["Votacao"]["item"], 0, 2) !=
                $this->request->data["Votacao"]["tr"]
            ) {
                $this->Flash->error(
                    __(
                        "Os dois primeiros dígitos do campo Item tem que ser iguais ao TR.",
                    ),
                );
                return $this->redirect([
                    "action" => "edit",
            ...
            if (isset($this->request->data["Votacao"]["votacao"])) {
                $votos = explode(
                    "/",
                    $this->request->data["Votacao"]["votacao"],
                );
            }
            ...
            if ($votos[1] >= $tercovotos) {
                $this->Flash->error(
                    __(
                        "Há uma votação minoritária. Não esqueça de registrar a votação minoritária.",
                    ),
                );
            }

            /** Verifica que a votação seja inserida utilizando como separador dos votos uma barra inclinada */
            $verificaponto = strpos(
                $this->request->data["Votacao"]["votacao"],
                ".",
            );
            if ($verificaponto) {
                // echo "Separar os valores com uma barra inclinada: '/'" . "<br>";
                $this->request->data["Votacao"]["votacao"] = str_replace(
                    ".",
                    "/",
                    $this->request->data["Votacao"]["votacao"],
                );
            }
            $verificabarra = strpos(
                $this->request->data["Votacao"]["votacao"],
                "/",
            );
            if (empty($verificabarra)) {
                ...
                $this->Flash->error(
                    __(
                        "Separar os três valores da votação com uma barra inclinada: '/'",
                    ),
                );
            } else {
                ...
                if (count($votos) != 3) {
                    ...
                    $this->Flash->error(
                        __(
                            "A votação tem que ter três valores separados por barra inclinada: '/'",
                        ),
                    );
                    return $this->redirect([
                        "controller" => "Items",
            ...
            if ($this->Votacao->save($this->request->data)):
                ...
                $this->Flash->error(
                    __("Votação não foi atualizada. Tente novamente."),
                );
            endif;
        }
        $options = [
            "conditions" => ["Votacao." . $this->Votacao->primaryKey => $id],
        ];
    ...
    public function add($id = null)
    {
        ...
        $item_id = isset($this->request->query["item_id"])
            ? $this->request->query["item_id"]
            : null;
        $votacao_id = isset($this->request->query["votacao_id"])
            ? $this->request->query["votacao_id"]
            : null;
        $resultado = isset($this->request->query["resultado"])
            ? $this->request->query["resultado"]
            : null;
        ...
        /* Se o resultado é minoritária, então seta a flag com o valor 0 para que não entre em loop. Acho que já não é mais necessário. */
        ...
        if (isset($item_id) && $item_id != null) {
            ...
            $options = [
                "conditions" => ["Item." . $this->Item->primaryKey => $item_id],
            ];
        ...
        if (isset($resultado) && $resultado == "minoritária"):
            // Se é uma votação minoritária obtenho a votacao realizada para recuperar os resultados
            ...
            if ($votacao_id):
                $options = [
                    "conditions" => [
                        "Votacao." . $this->Votacao->primaryKey => $votacao_id,
                    ],
                ];
                ...
                $this->Flash->error(__("Item sem votação?"));
                exit();
            endif;
        endif;
        ...
        if ($this->request->is("post")) {
            ...
            if ($this->Auth->user("role") == "admin"):
                $this->request->data["Votacao"]["user_id"] = $this->Auth->user(
                    "id",
                );
            ...
            if (empty($this->request->data["Votacao"]["item_modificada"])) {
                if ($this->request->data["Votacao"]["item_incluida"]) {
                    $this->request->data["Votacao"]["item_modificada"] =
                        $this->request->data["Votacao"]["item_incluida"];
                }

                if ($this->request->data["Votacao"]["item_minoritaria"]) {
                    $this->request->data["Votacao"]["item_modificada"] =
                        $this->request->data["Votacao"]["item_minoritaria"];
                }
            ...
            if (
                $this->request->data["Votacao"]["resultado"] == "inclusão" ||
                $this->request->data["Votacao"]["resultado"] == "minoritária"
            ) {
                ...
                $javotado = $this->Votacao->find("first", [
                    "conditions" => [
                        "Votacao.item_id" =>
                            $this->request->data["Votacao"]["item_id"],
                        "Votacao.grupo" =>
                            $this->request->data["Votacao"]["grupo"],
                        "Votacao.evento_id" => $evento_id,
                        "Votacao.resultado IN" => [
                            "aprovada",
                            "modificada",
                            "suprimida",
                            "remitida",
            ...
            if ($this->request->data["Votacao"]["tr_suprimida"] == 1):
                $this->suprimeTR(
                    $this->request->data["Votacao"]["tr_suprimida"],
                );
            endif;
            ...
            if ($this->request->data["Votacao"]["resultado"] == "modificada"):
                if (empty($this->request->data["Votacao"]["item_modificada"])):
                    $this->Flash->error(
                        __("Registre a alteração do item da TR."),
                    );
                    return $this->redirect([
                        "controller" => "Votacaos",
                        "action" => "add",
            ...
            if ($this->request->data["Votacao"]["resultado"] == "inclusão") {
                // die('inclusão');
                $this->request->data["Votacao"]["item"] =
                    $this->request->data["Votacao"]["tr"] . ".99";
            ...
            if ($verificaponto) {
                ...
                $this->Flash->error(
                    __(
                        "Separar os valores da votação com uma barra inclinada: '/'",
                    ),
                );
            }
            $verificabarra = strpos(
            ...
            if (
                $this->request->data["Votacao"]["resultado"] == "remitida" ||
                $this->request->data["Votacao"]["resultado"] == "outra"
            ) {
                if (empty($this->request->data["Votacao"]["observacoes"])) {
                    $this->Flash->error(
                        __(
                            "Especifique no campo 'Observações' o resultado da deliberação do item 'remitido' ou votado como 'outra'.",
                        ),
                    );
                    return $this->redirect([
                        "controller" => "Items",
            ...
            if ($this->Votacao->save($this->request->data)) {
                ...
                if ($flagminoritaria == "1") {
                    $this->Flash->success(
                        __("Votação inserida. Registre a votação minoritária"),
                    );
                    return $this->redirect([
                        "controller" => "Votacaos",
                        "action" => "add",
                        "?" => [
                            "item_id" =>
                                $this->request->data["Votacao"]["item_id"],
                            "votacao_id" => $this->Votacao->getLastInsertID(),
                            "resultado" => "minoritária",
                        ],
                    ]);
                } else {
                ...
                // die();
                $this->Flash->error(
                    __("Votação não foi inserida. Tente novamente."),
                );
            }
        }
    }
        ...
        if ($dados) {
            ...
            if ($this->request->data["Votacao"]["resultado"] == "inclusão") {
                ...
                if (!empty($outro_item["Item"]["id"])) {
                    ...
                    $this->Flash->error(
                        __("O item não existe. Inserir novo item na TR"),
                    );
                    return $this->redirect([
                        "controller" => "Items",
                        "action" => "add",
    ...
    public function aprovaembloco($dados)
    {
        ...
        if ($this->request->data["Votacao"]["tr_aprovada"] == 1) {
            ...
            foreach ($items as $c_item) {
                $this->request->data["Votacao"]["item"] = substr(
                    $c_item["Item"]["item"],
                    0,
                    5,
                );
    ...
    public function suprimeTR($suprime)
    {
        if ($this->request->data["Votacao"]["tr_suprimida"] == 1) {
            ...
            if ($this->request->data["Votacao"]["resultado"] !== "suprimida") {
                ...
                $this->Flash->error(
                    __(
                        'Tem que selecionar "suprimida" também na caixa "Resolução".',
                    ),
                );
                return $this->redirect([
                    "controller" => "Votacaos",
            ...
            if (!empty($this->request->data["Votacao"]["item_modificada"])) {
                $this->Flash->error(
                    __(
                        "O campo Item modificado não está vazio. Verifique antes de suprimir a TR",
                    ),
                );
                return $this->redirect([
                    "controller" => "Votacaos",
    ...
    public function usuario($data)
    {
        ...
        if ($this->Auth->user("role") === "admin") {
            ...
            $this->loadModel("User");
            $usuarioData = $this->User->find("first", [
                "conditions" => ["User.username" => $grupo],
            ]);
            $this->request->data["Votacao"]["user_id"] =
                $usuarioData["User"]["id"];
    ...
    public function view($id = null)
    {
        ...
        $votacao_grupo = $this->Votacao->findById($id, [
            "fields" => "Votacao.grupo",
        ]);
        // pr($votacao_grupo['Votacao']['grupo']);
        // die();
        if ($this->Auth->user("role") == "relator") {
            if (
                substr($this->Auth->user("username"), 5, 2) ==
                $votacao_grupo["Votacao"]["grupo"]
            ) {
                // echo "Usuario relator do mesmo grupo da votação autorizado";
                $options = [
                    "conditions" => [
                        "Votacao." . $this->Votacao->primaryKey => $id,
                    ],
                ];
                ...
                return $this->redirect([
                    "action" => "index",
                    "?" => ["grupo" => $votacao_grupo["Votacao"]["grupo"]],
                ]);
            }
        ...
        if (
            ...
            if ($this->Votacao->delete($id)) {
                ...
                if ($this->Auth->user("role") === "relator") {
                    ...
                    return $this->redirect([
                        "action" => "index",
                        "?" => ["grupo" => $votacao["Votacao"]["grupo"]],
                    ]);
                } elseif ($this->Auth->user("role") === "admin") {
                    /* Se eh admin vai para tr */
                    return $this->redirect([
                        "action" => "index",
                        "?" => ["tr" => $votacao["Votacao"]["tr"]],
                    ]);
                }
            } else {
                $this->Flash->error(
                    __("Votação não foi excluida. Tente novamente."),
                );
                if ($this->Auth->user("role") === "relator") {
                    /* Se eh relator vai para grupo */
                    return $this->redirect([
        ...
        if ($this->request->data) {
            ...
            for ($i = 0; $i < count($relatorio); $i++) {
                ...
                for ($t = 0; $t < count($relatorio[$i]); $t++):
                    ...
                    if (
                        $relatorio[$i][$t]["Votacao"]["resultado"] == "aprovada"
                    ):
                        // echo $i . " Aprovada: " . "<br>";
                        $taprovada .=
                            "Grupo " . $relatorio[$i][$t]["Votacao"]["grupo"];
                        $taprovada .= " ";
                        $taprovada .=
                            "item: " .
                            substr($relatorio[$i][$t]["Votacao"]["item"], 3, 2);
                        $taprovada .= " ";
                        $taprovada .=
                            "(" .
                            $relatorio[$i][$t]["Votacao"]["votacao"] .
                            ")";
                        $taprovada .= ", ";
                        $qaprovada++;
                    endif;
                    ...
                    if (
                        $relatorio[$i][$t]["Votacao"]["resultado"] ==
                        "modificada"
                    ):
                        // echo $i . "Modificada: ";
                        $tmodificada .=
                    ...
                    if (
                        $relatorio[$i][$t]["Votacao"]["resultado"] == "inclusão"
                    ):
                        // echo $i . "Inclusão: ";
                        $tincluida .=
                            "Grupo " . $relatorio[$i][$t]["Votacao"]["grupo"];
                    ...
                    if (
                        $relatorio[$i][$t]["Votacao"]["resultado"] == "remitida"
                    ):
                        // echo $i . "Inclusão: ";
                        $tremitida .=
                            "Grupo " . $relatorio[$i][$t]["Votacao"]["grupo"];
                    ...
                    $suprimidas_integralmente_sem_elementos_vazios = array_filter(
                        $suprimidas_integralmente,
                        "strlen",
                    );
                    $trs_suprimidas_integralmente = array_unique(
                        $suprimidas_integralmente_sem_elementos_vazios,
                    );
                    $trs_suprimidas_integralmente_string = implode(
                        ", ",
                        $trs_suprimidas_integralmente,
                    );
                ...
                $quantidade[] =
                    ...
                    $tr .
                    "</b>. " .
                    "<b>Suprimida integralmente:</b> " .
                    count($trs_suprimidas_integralmente) .
                    ", <b>Aprovados:</b> " .
                    $qaprovada .
                    ", <b>modificados:</b> " .
                    $qmodificada .
                ...
                $situacao_nos_grupos[] =
                    ...
                    "</b>. " .
                    "<b>" .
                    "Suprimida integralmente no(s) grupo(s): " .
                    "</b>" .
                    $suprimida_integralmente .
                    "." .
                    "<br />" .
                    $aprovada .
                    $modificada .
    ...
    public function collation()
    {
        ...
        foreach ($votacoes as $votacao) {
            ...
            $votacaonova["Votacao"]["evento_id"] =
                $votacao["Votacao"]["evento_id"];
            $votacaonova["Votacao"]["grupo"] = $votacao["Votacao"]["grupo"];
            $votacaonova["Votacao"]["tr"] = $votacao["Votacao"]["tr"];
            $votacaonova["Votacao"]["tr_suprimida"] =
                $votacao["Votacao"]["tr_suprimida"];
            $votacaonova["Votacao"]["tr_aprovada"] =
                $votacao["Votacao"]["tr_aprovada"];
            $votacaonova["Votacao"]["item_id"] = $votacao["Votacao"]["item_id"];
            $votacaonova["Votacao"]["item"] = $votacao["Votacao"]["item"];
            $votacaonova["Votacao"]["resultado"] =
                $votacao["Votacao"]["resultado"];
            $votacaonova["Votacao"]["resultado1"] =
                $votacao["Votacao"]["resultado"];
            $votacaonova["Votacao"]["votacao"] = $votacao["Votacao"]["votacao"];
            $votacaonova["Votacao"]["item_modificada"] = str_replace(
                ["\r", "\n"],
                "",
                $votacao["Votacao"]["item_modificada"],
            );
            $votacaonova["Votacao"]["item_modificada1"] = str_replace(
                ["\r", "\n"],
                "",
                $votacao["Votacao"]["item_modificada"],
            );
            $votacaonova["Votacao"]["data"] = $votacao["Votacao"]["data"];
            $votacaonova["Votacao"]["observacoes"] =
                $votacao["Votacao"]["observacoes"];
    ...
    public function insercao()
    {
        ...
        foreach ($insercao as $inserir) {
            ...
            $inserirItem["Item"]["texto"] = str_replace(
                ["\r", "\n"],
                "",
